...
export default function App() {
  ...
  useEffect(() => {
    fetch(API_URL)
      ...
      .then(data => {
        ...
        // 修改：获取订单号并按数字大小降序排列
        const sortedOrderKeys = Object.keys(grouped).sort((a, b) => Number(b) - Number(a));
        
        // 默认选中最大的订单号
        if (sortedOrderKeys.length > 0) {
          setActiveOrderIds([sortedOrderKeys[0]]);
        }
        setIsLoading(false);
      })
      .catch(error => {
  ...
  // 新增：合并多个订单的图片并根据 SKU 进行去重
  const getMergedAndDedupedItems = () => {
    const combinedItems = [];
    const seenSkus = new Set();

    activeOrderIds.forEach(id => {
      const items = ordersData[id] || [];
      items.forEach(item => {
  ...
  const handleDragEnd = (event) => {
    ...
    // 从去重后的合并列表中寻找拖拽的图片
    const draggedItem = getMergedAndDedupedItems().find(item => item.id === imageId);
      ...
      <div className="app-container">
        <div className="sidebar-container">
          <Sidebar 
            ...
            activeOrderItems={getMergedAndDedupedItems()} 
